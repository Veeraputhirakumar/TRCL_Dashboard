...
def run_dash():
    ...
    app.layout = html.Div(children=[
        ...
        html.Div([
            ...
            # Trend Selection Dropdown and Options Dropdown (nearby)
            html.Div([
                html.Div([
                ...
                html.Div(id='options-container', style={'display': 'none', 'margin-top': '20px'})
        ...
        html.Div(id='date-range-container', style={'display': 'none', 'margin-top': '20px'}),
    ...
    # Callback to show Date Range input fields based on Option selection
    @app.callback(
    ...
    def display_date_range(selected_option):
        if selected_option:
            return html.Div([
    ...
    def update_graph(start_date, end_date, day, month, year, selected_mill, selected_option, trend_type):
        ...
        # Convert input dates for specific date
        if trend_type == 'Specific Date' and day and month and year:
            try:
                specific_date = pd.to_datetime(f'{year}-{month:02d}-{day:02d}', format='%Y-%m-%d')
            except ValueError:
                ...
                return dcc.Graph(
                    ...
                    figure={
                        'data': [
                            go.Bar(
                                x=[selected_option],
                                y=[filtered_df[selected_option].sum()],
                                marker_color='blue'
                            )
                        ],
                        'layout': go.Layout(
        ...
        # Convert input dates for range of dates
